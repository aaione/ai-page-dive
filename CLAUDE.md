...
Chrome 扩展（MV3）：一键调用本机已装的 AI CLI（claude / codex）深度总结当前网页。
...
pnpm monorepo：`apps/extension`（Vite + React + TS + Tailwind v4 + react-markdown）+ `packages/host`（纯 Node >=18 + TS，运行时零依赖）+ `packages/shared`（AgentDef 类型 / NM 消息协议）。测试 vitest（host 流解析器 + buildArgs 纯函数）。
